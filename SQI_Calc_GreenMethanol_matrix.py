...
class Via(object):
    ...
    def optimize_self(self, speedy=False, 
        ...
        while not self.met_concentrate_tds():
            ### estimates the correct number of stages to add to speed up the optimization
            z=self.stage_df_list[0]['Concentrate TDS'].to_numpy().copy()
            if len(z)==1:
                how_many_stages_to_add =1 
            else:
                how_many_stages_to_add = int((self.concentrate_tds_target-z[-1])/(z[-1]-z[-2]))
            if how_many_stages_to_add<1:
                how_many_stages_to_add=1 
            self.num_stages[0]+=how_many_stages_to_add
            
            self.massbalance()
            
            if self.met_concentrate_tds():
                print('Concentrate TDS met', self.get_concentrate_output()[1], 'vs.', self.concentrate_tds_target)
            else:
                print("Conc TDS", self.get_concentrate_output()[1], 'vs.', self.concentrate_tds_target)
         
                
        print("First Pass build complete", self.num_stages)
        
       
        
        print("Beginning Pass addition.  Will add passes until meeting recovery spec")
        
        
        # while not self.met_permeate_tds():
        #     print("building next pass", self.calc_solute_recovery(), self.num_stages)
        #     self.num_stages.append(1) 
        #     self.massbalance(passback_config=True)

        #     if len(self.df.index)>3:
        #         break

        #     while self.stage_df_list[-1]['Concentrate TDS'].iloc[-1]<self.stage_df_list[-2]['Feed TDS'][0]:
                
        #         z=self.stage_df_list[-1]['Concentrate TDS'].to_numpy().copy()
        #         tds_target = self.stage_df_list[-2]['Feed TDS'][0]
        #         if len(z)==1:
        #             how_many_stages_to_add =1 
        #         else:
        #             how_many_stages_to_add = int((tds_target-z[-1])/(z[-1]-z[-2]))
        #         if how_many_stages_to_add<1:
        #             how_many_stages_to_add=1 
        #         self.num_stages[-1]+=how_many_stages_to_add
    ...
    def consolidate_permeate_connectivity(self):
        ...
        for target in np.linspace(min(self.stage_df['Feed TDS']), max(self.stage_df['Feed TDS']), 5)[0:4]:
    ...
    def massbalancestage(self, pass_num,number_of_stages):
        ...
        self.stage_df.loc[0,'Feed TDS']=self.permeate_tds_target
        ...
        for cycle in range(1):
            ...
            permeate_of_later_stage_mixed_with_permeates_of_earlier_stage=False
            the_permeate_are_combined_only_before_next_pass=True
            
            if permeate_of_later_stage_mixed_with_permeates_of_earlier_stage:
                for stage_i in np.arange(number_of_stages, ):
                    
                    self.stage_df.loc[stage_i, 'Permeate TDS'] =(self.stage_df.loc[stage_i:, 'Permeate Flow']*self.stage_df.loc[stage_i:, 'Permeate TDS']).sum()/self.stage_df.loc[stage_i:, 'Permeate Flow'].sum()
                    self.stage_df.loc[stage_i, 'Permeate Flow'] =self.stage_df.loc[stage_i:, 'Permeate Flow'].sum()
    ...
    def massbalance(self,setfeedflow=False, setfeedtds=False, passback_config=True):
        ...
        while True:
            ...
            # correct_number_of_stages =False
            
            # while correct_number_of_stages==False:
            ...
            p_after_mass_bal = copy.deepcopy(self.stage_df['Permeate Flow'].to_numpy())
            
            self.set_permeate_connectivity()
            try:
                self.consolidate_permeate_connectivity()
            except:
                raise
            
            f,p,c, permeate_connections=solve_flows(self.stage_df, self.get_feed_input()[0], self.get_feed_input()[1])
           
            convergence_metric1 = p-self.stage_df['Permeate Flow'].to_numpy()
            # print(convergence_metric1, 'Flow')
            
            self.stage_df['Feed Flow']=f
            self.stage_df['Concentrate Flow']=c
            self.stage_df['Permeate Flow']=p
            self.stage_df['True Permeate Flow']=self.stage_df['Feed Flow']*self.stage_df['True Permeate Ratio']
            self.stage_df.loc[:,'Permeate Connections']=permeate_connections
            
            # self.set_permeate_connectivity()
            # try:
            #     self.consolidate_permeate_connectivity()
            # except:
            #     print('consolidate fail')
            #     raise
            f,p,c=solve_conc(self.stage_df, self.get_feed_input()[0], self.get_feed_input()[1])
            convergence_metric2 = p.to_numpy()-self.stage_df['Permeate TDS'].to_numpy()
            # print(convergence_metric2, 'TDS')
            self.stage_df['Feed TDS']=f
            self.stage_df['Concentrate TDS']=c
            self.stage_df['Permeate TDS']=p
            ...
            print(np.max(np.abs(convergence_metric1)),np.max(np.abs(convergence_metric2)))
            if cycle>10:
                # print(np.max(np.abs(convergence_metric1)),np.max(np.abs(convergence_metric2)))
                break
            
            if np.max(np.abs(convergence_metric1))<0.01 and np.max(np.abs(convergence_metric2))<0.0001:
                print('Exiting after cycles', cycle)
                break
            continue
    ...
    def check_solution(self):
        ...
        osmotic_pressure = calc_osmotic_pressure(s['Concentrate TDS'], s['Permeate TDS'], self.MW_solute, self.MW_solvent, )
...
def create_data(feed_rate_mt_year):
    ...
    input_tds = [0.1]#np.arange(0.2,1,0.1)
    concentrate_tds_target = [0.859,]#0.7,0.8,0.9]
...
if __name__== "__main__":
    ...
    feed_rate_mt_year =183000 ###MT/year of spent acid
